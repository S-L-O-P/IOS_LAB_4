...
*Приём - прерывание `ISR(USART_RX_vect)`.* Каждый принятый байт обрабатывается конечным автоматом с четырьмя состояниями (S\_SYNC -> S\_LEN -> S\_DATA -> S\_CRC). При совпадении CRC пакет помечается готовым к эхо-ответу; битый пакет отбрасывается, автомат возвращается в S\_SYNC
